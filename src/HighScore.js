...
class HighScore extends Character {
	constructor({
		...
		const maxScoreLength = Math.max(highScoresList.reduce((accumulator, [,score]) => {
		...
		for (let i = 0; i < highScoresList.length; i++) {
			...
			console.log(maxScoreLength, scoreString.length, maxScoreLength - scoreString.length);
			const scorePadding = new Array(maxScoreLength - scoreString.length).fill(' ').join('');
